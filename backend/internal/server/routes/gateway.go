...
func RegisterGatewayRoutes(
	...
	imageSequenceHandler := func(c *gin.Context) {
		if !isOpenAIGatewayPlatform(c) {
			service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
			c.JSON(http.StatusNotFound, gin.H{
				"error": gin.H{
					"type":    "not_found_error",
					"message": "Image sequences are not supported for this platform",
				},
			})
			return
		}
		h.OpenAIGateway.ImageSequence(c)
	}
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		gateway.POST("/images/sequences", imageSequenceHandler)
	...
	r.POST("/images/sequences", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), requireGroupAnthropic, imageSequenceHandler)
	r.GET("/images/jobs/:job_id", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), requireGroupAnthropic, h.OpenAIGateway.GetImageJob)
	r.GET("/images/jobs/:job_id/results/:index", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), requireGroupAnthropic, h.OpenAIGateway.GetImageJobResult)
	r.DELETE("/images/jobs/:job_id", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), requireGroupAnthropic, h.OpenAIGateway.CancelImageJob)
...
// gatewayImagesHandler keeps every browser and API entrypoint on the same
// platform dispatch path.
func gatewayImagesHandler(h *handler.Handlers) gin.HandlerFunc {
