...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	imageJobRepository := repository.NewImageJobRepository(client, db)
	imageJobObjectStore, err := repository.ProvideImageJobObjectStore(configConfig)
	if err != nil {
		return nil, err
	}
	...
	openAIGatewayHandler := handler.NewOpenAIGatewayHandler(openAIGatewayService, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, openAIImageExecutor, imageJobService, configConfig)
	...
	imageEditorRepository := repository.NewImageEditorRepository(db)
	imageCanvasProjectService := service.NewImageCanvasProjectService(imageCanvasRepository, imageJobObjectStore)
	imageEditorService := service.NewImageEditorService(imageEditorRepository, imageCanvasProjectService)
	...
	imageCanvasHandler := handler.ProvideImageCanvasHandler(imageCanvasProjectService, imageCanvasJobService, imageModelPolicyService, imageModelCatalog, canvasMediaService, imageEditorService)
