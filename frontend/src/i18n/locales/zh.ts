export default {
  ...
  keys: {
    ...
    createdKeyTitle: '保存新 API 密钥',
    createdKeyWarning: '继续前请复制并妥善保存此 API 密钥。',
    copyCreatedKey: '复制 API 密钥',
    continueToStudio: '继续前往 Studio',
    createdKeyDone: '完成',
  ...
  admin: {
    imageCanvas: {
      ...
      openStudio: '打开创作工作台',
      policyTitle: '有序图片模型链',
      policyDescription: '展示策略中的全部媒体模型及当前调度覆盖；顺序决定图片任务回退，音视频仅使用启停状态。',
      enabled: '启用画布图片任务',
      enabledDescription: '允许登录用户从画布创建图片任务。',
      modelEnabled: '启用',
      generation: '生成',
      edit: '编辑',
      moveUp: '上移',
      ...
      availableModelsDescription: '将当前可调度模型加入链尾。',
      noModels: '当前没有可调度的图片模型。',
      atLeastOne: '至少需要启用一个模型。',
      auditTitle: '最近策略变更',
      conflict: '策略已被其他管理员更新，已加载最新版本。',
      runtimeTitle: '运行策略与参数预设',
      runtimeDescription: '调整异步任务容量、结果保留期和模型参数预设。',
      restoreDefaults: '恢复启动默认值',
      workerState: 'Worker 状态',
      workerStopped: '未启动',
      workerCount: '{running} / {target} 个 Worker',
      storageState: '对象存储',
      ...
      permanentAssetNotice: '画布成品永久保存；结果保留期只作用于临时任务对象。',
      temporaryAssetNotice: '任务对象会按结果保留期清理，请及时保存需要长期保留的成品。',
      limitsTitle: '任务容量与保留',
      workerConcurrency: '全局 Worker 并发',
      workerConcurrencyHint: '缩容会等待当前任务结束。',
      maxActiveJobs: '每用户最大活动任务',
      maxActiveJobsHint: '包含排队中和运行中的任务。',
      taskTimeout: '单任务总时限（秒）',
      currentDuration: '当前：{duration}',
      maxOutputs: '单任务最大输出数',
      maxInputs: '单任务最大参考图数',
      resultRetention: '临时结果保留期（秒）',
      concurrencyNotice: '提高并发会增加上游请求和服务器内存占用；缩容不会取消正在生成的图片。',
      defaultsTitle: '模型默认参数',
      defaultsDescription: '覆盖模型目录中的默认尺寸、质量、格式和背景。',
      addDefault: '添加模型默认值',
      noDefaultOverrides: '当前使用模型目录的内建默认值。',
      presetsTitle: '自定义参数预设',
      presetsDescription: '为指定模型添加可在用户画布中选择的参数组合。',
      addPreset: '添加预设',
      noCustomPresets: '当前没有自定义预设。',
      model: '模型',
      presetId: '预设 ID',
      presetLabel: '显示名称',
      presetDefaultLabel: '自定义预设 {number}',
      experimentalPreset: '标记为实验性预设',
      size: '像素尺寸',
      aspectRatio: '宽高比',
      resolution: '分辨率',
      quality: '生成质量',
      format: '输出格式',
      background: '背景',
      compression: '压缩质量（0-100）',
      noDefault: '不设默认值',
      customSizeHint: '最长边不超过 {maxEdge}px，宽高须为 {multiple} 的倍数，宽高比不超过 {maxRatio}:1。',
      experimentalSize: '{model} 的这个尺寸属于实验能力。',
      invalidLimits: '运行限制必须是允许范围内的整数。',
      tooManyOverrides: '模型默认值或自定义预设数量超过服务端上限。',
      unknownModel: '模型 {model} 当前不可调度。',
      duplicateDefault: '模型 {model} 只能配置一组默认参数。',
      invalidPreset: '预设 ID 仅支持字母、数字、点、下划线和连字符，显示名称不能为空。',
      duplicatePreset: '模型 {model} 已存在 ID 为 {id} 的预设。',
      invalidParameters: '模型 {model} 的参数组合无效。',
      ...
      video: '视频',
      audio: '音频',
      reasonGroupFilter: '被分组模型白名单过滤',
      reasonNoGroup: '没有可用的账号分组',
      reasonUnknown: '未知模型',
      reasonNoAccount: '没有可调度账号',
      addModel: '加入模型链',
      selectModel: '选择可调度模型',
      addToChain: '加入链尾',
      disableUnavailableModels: '请先停用或删除不可调度的模型，再保存策略。',
      fallbackTitle: '图片任务回退规则',
      fallbackDescription: '图片任务中用户选中的模型永远优先；只有最终没有产出且错误可重试时才会进入后续模型。音频和视频只校验模型是否启用。',
      fallbackSelectedFirst: '选中模型优先',
      fallbackSameProvider: '仅同供应商兼容回退',
      fallbackRetryable: '仅 0、429、5xx 可重试',
      fallbackAccountFirst: '先做同模型账号切换',
      fallbackPolicyNo: '内容策略拦截不会回退',
      auditVersion: '版本',
      auditOperator: '操作者',
      auditChange: '变更摘要',
      auditTime: '时间',
      auditSwitchChanged: '策略开关改为{state}',
      auditModelsChanged: '模型链 {before} 项改为 {after} 项',
      auditVersionOnly: '策略版本更新',
      recentJobsTitle: '最近图片任务',
      recentJobsDescription: '查看实际候选链、尝试耗时和最终使用的模型。',
      noRecentJobs: '暂无图片任务记录。',
      job: '任务',
      jobStatus: '状态',
      jobStatus_queued: '排队中',
      jobStatus_running: '运行中',
      jobStatus_completed: '已完成',
      jobStatus_partial: '部分完成',
      jobStatus_failed: '失败',
      jobStatus_indeterminate: '状态不确定',
      jobStatus_canceled: '已取消',
      jobStatus_expired: '已过期',
      jobModel: '模型',
      jobAttempts: '尝试 / 计划',
      jobOutput: '产出',
      jobCreated: '创建时间',
      collapseAttempts: '收起尝试详情',
      expandAttempts: '展开尝试详情',
      attemptPlan: '候选计划',
      attemptLog: '实际尝试',
      noAttempts: '尚未记录尝试。',
      outputsShort: '张',
      hideAttempts: '收起详情',
      viewAttempts: '查看尝试',
      backendTitle: '后端逻辑与数据访问',
      backendDescription: '创作工作台当前使用的准入、调度、持久化对象和参数化查询。',
      logicFlow: '执行链路',
      dataObjects: '数据对象',
      sqlQueries: 'SQL 查询',
      flowGate: '入口准入',
      flowGateDetail: '读取用户入口开关；关闭时在路由中间件直接拒绝新请求。',
      flowConfig: 'Key 与模型',
      flowConfigDetail: '校验 Key 所有权、状态和分组，再返回该分组实际可用的模型能力。',
      flowPolicy: '候选计划',
      flowPolicyDetail: '图片任务把所选模型置于首位并构建同供应商回退链；音视频任务只校验所选模型已启用。',
      flowReserve: '额度预留',
      flowReserveDetail: '校验并发上限、估算候选模型费用，在事务内预留额度并写入排队任务。',
      flowWorker: '执行与回退',
      flowWorkerDetail: 'Worker 先切换同模型账号；仅在无最终产出且错误可重试时进入下一模型。',
      flowPersist: '结果持久化',
      flowPersistDetail: '保存结果对象、图片资产和画布引用，并结算或释放预留额度。',
      objectName: '对象 / 表',
      objectPurpose: '职责',
      keyFields: '关键字段',
      ownerModule: '所属模块',
      objectSettings: '控制用户画布路由和新任务入口，不等同于模型策略开关。',
      objectKeys: '用户选择的本地调用凭证；关联用户、分组、状态和额度，不返回明文 Key。',
      objectGroups: '定义图片能力准入、模型白名单和计费口径。',
      objectAccounts: '上游供应商账号及分组绑定，是模型能力和实际调度覆盖的来源。',
      objectPolicy: '保存全局模型顺序、单模型启停和乐观锁版本。',
      objectAudit: '记录每次策略更新的操作者及更新前后快照。',
      objectJobs: '保存异步任务状态、模型候选计划、实际尝试和最终成功模型。',
      objectResults: '保存上游输出、对象存储位置、永久资产及项目引用关系。',
      queryEntry: '读取用户入口开关',
      queryCatalog: '计算账号 / 分组 / Key 覆盖',
      queryPolicy: '读取策略与审计',
      queryCreateJob: '预留并创建任务',
      queryWorker: '并发安全领取任务',
      queryAttempts: '记录模型尝试',
      queryRecentJobs: '读取最近任务'
    },
    ...
    settings: {
      ...
      features: {
        imageCanvas: {
          title: '图片画布',
          description: '向用户开放服务端持久化的图片生成画布。默认关闭。',
          enabledHint: '关闭时隐藏用户侧入口，并拒绝创建新的画布图片任务。',
        },
        channelMonitor: {
          title: '渠道监控',
