...
export default {
    ...
    assets: {
        migrationPartial: "有 {{count}} 个本地资产暂未迁移，将在下次打开时重试",
        title: "我的资产",
        description: "集中管理常用文本、图片、视频和音频资产。",
    ...
    canvas: {
        ...
        apiKeys: {
            select: "选择 API Key",
            manage: "管理 API Key",
            empty: "当前账户还没有 API Key",
            emptyTitle: "创建 API Key 后使用 Studio",
            emptyDescription: "Studio 使用你账户下的 Sub2API Key 进行权限校验、模型授权和账单归属。",
            create: "创建 API Key",
            unavailableTitle: "没有可用于 Studio 的 API Key",
            unavailableDescription: "请检查 API Key 及其分组。Studio 需要一个有效且有权使用已启用图片模型的 Key。",
            configErrorTitle: "无法加载 Studio 配置",
            configErrorDescription: "请检查网络后重试。你的项目不会受到影响。",
            retry: "重试",
            noGroup: "未绑定分组",
            policyDisabled: "图片任务总开关尚未启用",
            reasons: {
                key_disabled: "Key 已停用",
                key_expired: "Key 已过期",
                quota_exhausted: "Key 额度已用尽",
                group_missing: "未绑定可用分组",
                group_disabled: "所属分组已停用",
                image_generation_disabled: "所属分组未开启生图",
                no_image_model: "所属分组没有可用图片模型",
                unknown: "当前不可用于创作",
            },
        },
        ...
        sidePanel: {
            ...
            addingAssets: "正在添加资产…", addedAssets: "已添加 {{count}} 个资产", mediaOnly: "仅支持图片、视频或音频文件", addFailed: "添加失败，请重试", searchAssets: "搜索资产", add: "添加", noAssets: "暂无资产", inserted: "插入画布", removeAssetTitle: "移除该资产？", remove: "移除", removeAsset: "移除资产", assetRemoved: "资产已移除", removeFailed: "资产移除失败，请重试",
        ...
        projectPage: {
            ...
            uploadFailedFallback: "请检查文件格式、大小或网络后重试",
            noAudioToSave: "没有可保存的音频",
            canvasAudio: "画布音频",
            assetSaveFailed: "资产保存失败：{{reason}}",
            assetSaveFailedFallback: "请检查网络后重试",
            assetInsertFailed: "读取资产失败：{{reason}}",
            assetInsertFailedFallback: "请检查网络后重试",
            saveDirty: "未保存",
            saveSaving: "保存中",
            saveSaved: "已保存",
            saveError: "仅本地",
        ...
        focusedEditor: {
            title: "专注图片编辑器", back: "返回画布", apply: "应用", image: "编辑后的图片", loading: "正在加载编辑器", unavailable: "无法打开编辑器", persistedRequired: "请从画布中选择一张已持久化的图片节点。", loadFailed: "编辑器加载失败", modelRequired: "请先选择图片模型和 API Key", maskUnavailable: "当前模型不支持蒙版编辑", promptRequired: "请输入扩展区域的内容描述", nodeMissing: "来源图片节点已不存在", applied: "编辑结果已应用到画布", revisions: "版本", revision: "版本 {{number}}", source: "原图", transform: "编辑", crop: "裁剪", maskEdit: "蒙版编辑", backgroundReplace: "替换背景", outpaint: "扩图", outpaintPrompt: "描述扩展区域需要补全的内容", expand: "扩展画布", view: "视图", transparent: "透明", white: "白色", black: "黑色", job: "图片任务", job_idle: "空闲", job_running: "处理中", job_completed: "已完成", job_failed: "失败", job_canceled: "已取消", cancelJob: "取消任务", conflict: "编辑器已在其他位置更新", retrySave: "重试本地保存", reload: "使用服务器版本", retry: "重试", zoomOut: "缩小", zoomIn: "放大", resetZoom: "重置缩放", save_idle: "就绪", save_saving: "保存中", save_saved: "已保存", save_conflict: "版本冲突", save_error: "保存失败"
        },
