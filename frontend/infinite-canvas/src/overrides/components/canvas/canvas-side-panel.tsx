        ...
        try {
            for (const file of files) {
                if (file.type.startsWith("image/")) {
                    ...
                    await addAsset({ kind: "video", title: file.name || t("assets.kinds.video"), coverUrl: "", tags: [], data: { url: media.url, storageKey: media.storageKey, width: media.width || 0, height: media.height || 0, bytes: media.bytes, mimeType: media.mimeType, durationMs: media.durationMs } });
                    added += 1;
                } else if (file.type.startsWith("audio/")) {
                    const media = await uploadMediaFile(file, "audio");
                    await addAsset({ kind: "audio", title: file.name || t("assets.kinds.audio"), coverUrl: "", tags: [], data: { url: media.url, storageKey: media.storageKey, bytes: media.bytes, mimeType: media.mimeType, durationMs: media.durationMs } });
                    added += 1;
                }
            }
                                                ...
                                                <AssetCard
                                                    ...
                                                    onRemove={async () => {
                                                        try {
                                                            ...
                                                            console.error(error);
                                                            message.error(t("canvas.sidePanel.removeFailed"));
                                                        }
                                                    }}
                                                />
                                            ))}
...
function AssetCover({ asset }: { asset: Asset }) {
    ...
    if (asset.kind === "audio") return <div className="grid size-full place-items-center"><Music2 className="size-8 opacity-55" /></div>;
    if (asset.kind === "video") {
