...
import { BookOpen, Bot, Check, CloudOff, Copy, Download, Home, Images, LoaderCircle, Menu, PanelLeftClose, PanelLeftOpen, Plus, Redo2, RefreshCw, Save, Trash2, TriangleAlert, Undo2, Upload } from "lucide-react";
...
export function CanvasTopBar({
    ...
    canUndo: boolean;
    canRedo: boolean;
    saveState: "dirty" | "saving" | "saved" | "error" | "conflict";
    saveError?: string;
    onSaveNow: () => void;
    onRetrySave: () => void;
    onReloadServer: () => void;
...
function CanvasSaveStatus({
    ...
    const { t } = useTranslation();
    const labels = {
        dirty: t("canvas.projectPage.saveDirty"),
        saving: t("canvas.projectPage.saveSaving"),
        saved: t("canvas.projectPage.saveSaved"),
        error: t("canvas.projectPage.saveError"),
        conflict: t("canvas.projectPage.saveConflict"),
    };
    const icons = {
        dirty: <Save className="size-3.5" />,
        saving: <LoaderCircle className="size-3.5 animate-spin" />,
        saved: <Check className="size-3.5" />,
        error: <CloudOff className="size-3.5" />,
        conflict: <TriangleAlert className="size-3.5" />,
    };
    const control = (
        <button
            type="button"
            className={`canvas-save-status canvas-save-status-${state}`}
            aria-label={labels[state]}
            onClick={state === "dirty" ? onSaveNow : undefined}
        >
            {icons[state]}
    ...
    if (state !== "error" && state !== "conflict") return control;
    return (
        <Tooltip title={error || labels[state]}>
            <Dropdown
                trigger={["click"]}
                menu={{
                    items: state === "conflict"
                        ? [
                              { key: "reload", icon: <RefreshCw className="size-4" />, label: t("canvas.projectPage.reloadServer"), onClick: onReloadServer },
                              { key: "copy", icon: <Copy className="size-4" />, label: t("canvas.projectPage.saveAsCopy"), onClick: onSaveCopy },
                          ]
                        : [
                              { key: "retry", icon: <RefreshCw className="size-4" />, label: t("canvas.projectPage.retrySave"), onClick: onRetry },
                              { key: "copy", icon: <Copy className="size-4" />, label: t("canvas.projectPage.saveAsCopy"), onClick: onSaveCopy },
                          ],
                }}
            >
