// Sub2API-owned Shadow DOM compatibility override for the pinned upstream module.
...
export function CanvasPromptChipInput({ value, references, onChange, onSubmit, className, style, placeholder }: Props) {
    ...
    useEffect(() => {
        ...
        if (!editor) return;
        if (isEditorFocused(editor) && value === lastEmittedRef.current) return;
        editor.textContent = "";
...
function isEditorFocused(editor: HTMLElement) {
    const root = editor.getRootNode();
    return root instanceof ShadowRoot ? root.activeElement === editor : editor.ownerDocument.activeElement === editor;
}
