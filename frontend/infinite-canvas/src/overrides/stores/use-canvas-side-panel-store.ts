...
export const CANVAS_SIDE_PANEL_COMPACT_WIDTH = 900;
...
function compactViewport() {
    ...
    if (typeof window.matchMedia === "function") {
        return window.matchMedia(`(max-width: ${CANVAS_SIDE_PANEL_COMPACT_WIDTH}px)`).matches;
    }
    return window.innerWidth <= CANVAS_SIDE_PANEL_COMPACT_WIDTH;
}
...
function desktopPreferenceOpen() {
    if (typeof window === "undefined") return true;
    return localStorage.getItem(OPEN_KEY) !== "0";
}

const startsCompact = compactViewport();
const startsOpen = !startsCompact && desktopPreferenceOpen();
...
type CanvasSidePanelStore = {
    ...
    closePanel: () => void;
    togglePanel: () => void;
    setResponsiveCompact: (compact: boolean) => void;
};

export const useCanvasSidePanelStore = create<CanvasSidePanelStore>((set, get) => ({
    width: initialWidth(),
    panelOpen: startsOpen,
    panelMounted: startsOpen,
    panelClosing: false,
    compact: startsCompact,
    ...
    setResponsiveCompact: (compact) => {
        if (compact === get().compact) return;
        if (compact) {
            set({ compact: true, panelOpen: false, panelMounted: false, panelClosing: false });
            return;
        }
        const open = desktopPreferenceOpen();
        set({ compact: false, panelOpen: open, panelMounted: open, panelClosing: false });
    },
}));
