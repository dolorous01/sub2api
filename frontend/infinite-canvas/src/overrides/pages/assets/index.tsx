...
export default function AssetsPage() {
    ...
    const updateAsset = useAssetStore((state) => state.updateAssetAsync);
    const removeAsset = useAssetStore((state) => state.removeAssetAsync);
    ...
    const readImageFile = async (file?: File) => {
        ...
        try {
            ...
            console.error(error);
            message.error(t("assets.uploadFailed"));
        } finally {
    ...
    const downloadImage = async (asset: Asset) => {
        ...
        const url = asset.kind === "image"
            ? await resolveImageUrl(asset.data.storageKey, asset.data.dataUrl)
            : await resolveMediaUrl(asset.data.storageKey, asset.data.url);
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            console.error(error);
            message.error(t("assets.deleteFailed"));
        } finally {
            setDeleting(false);
        }
    };
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden bg-background text-stone-900 dark:text-stone-100">
            ...
            <Modal title={editingAsset ? t("assets.edit") : t("assets.add")} open={isAssetOpen} width={980} onCancel={() => setIsAssetOpen(false)} onOk={() => void saveAsset()} okText={t("common.save")} cancelText={t("common.cancel")} confirmLoading={savingAsset} destroyOnHidden>
            ...
            <Modal title={t("assets.deleteTitle")} open={Boolean(deletingAsset)} onCancel={() => setDeletingAsset(null)} onOk={() => void confirmDelete()} okText={t("common.delete")} okButtonProps={{ danger: true, loading: deleting }} cancelText={t("common.cancel")}>
                    ...
                    <Space>
                        ...
                        {asset.kind === "image" || asset.kind === "video" || asset.kind === "audio" ? (
                            <Button type="primary" icon={<Download className="size-4" />} onClick={() => onDownload(asset)}>
                                {asset.kind === "video" ? t("assets.downloadVideo") : asset.kind === "audio" ? t("common.download") : t("assets.downloadImage")}
                            </Button>
                        ) : null}
                    </Space>
                </div>
