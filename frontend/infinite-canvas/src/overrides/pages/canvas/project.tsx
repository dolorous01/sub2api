// Sub2API-owned project workflow override for the pinned upstream module.
...
function InfiniteCanvasPage() {
    ...
    const addAsset = useAssetStore((state) => state.addAssetAsync);
    ...
    const retryProject = useCanvasStore((state) => state.retryProject);
    const reloadServerProject = useCanvasStore((state) => state.reloadServerProject);
    const saveProjectAsNew = useCanvasStore((state) => state.saveProjectAsNew);
    const projectSaveState = useCanvasStore((state) => state.saveStatusByProject[projectId]?.state || "saved");
    const projectSaveError = useCanvasStore((state) => state.saveStatusByProject[projectId]?.error);
    ...
    const [serverReloadRevision, setServerReloadRevision] = useState(0);
    ...
    }, [hydrated, navigate, openProject, projectId, serverReloadRevision]);
    ...
    const uploadWithFeedback = useCallback(
        async <T,>(operation: () => Promise<T>): Promise<T | null> => {
            const hide = message.loading(t("canvas.projectPage.uploading"), 0);
            try {
                return await operation();
            } catch (error) {
                const reason = error instanceof Error && error.message ? error.message : t("canvas.projectPage.uploadFailedFallback");
                message.error(t("canvas.projectPage.uploadFailed", { reason }));
                return null;
            } finally {
                hide();
            }
    ...
    const createImageFileNode = useCallback(async (file: File, position: Position): Promise<boolean> => {
        const image = await uploadWithFeedback(() => uploadImage(file));
    ...
    const createVideoFileNode = useCallback(async (file: File, position: Position): Promise<boolean> => {
        const video = await uploadWithFeedback(() => uploadMediaFile(file, "video"));
    ...
    const createAudioFileNode = useCallback(async (file: File, position: Position): Promise<boolean> => {
        const audio = await uploadWithFeedback(() => uploadMediaFile(file, "audio"));
    ...
    const pasteSystemClipboard = useCallback(async () => {
        ...
        if (imageItem) {
            ...
            if (await createImageFileNode(file, getCanvasCenter())) {
    ...
    const saveNodeAsset = useCallback(
        async (node: CanvasNodeData) => {
            try {
                ...
                const reason = error instanceof Error ? error.message : t("canvas.projectPage.assetSaveFailedFallback");
                message.error(t("canvas.projectPage.assetSaveFailed", { reason }));
            }
        },
        [addAsset, message, t],
    );
    ...
    const handleImageInputChange = useCallback(
        async (event: ReactChangeEvent<HTMLInputElement>) => {
            ...
            if (target?.nodeId) {
                ...
                if (isAudioFile(first)) {
                    const audio = await uploadWithFeedback(() => uploadMediaFile(first, "audio"));
                    if (audio) {
                        const spec = NODE_DEFAULT_SIZE[CanvasNodeType.Audio];
                        setNodes((prev) =>
                            prev.map((node) =>
                    ...
                    const video = await uploadWithFeedback(() => uploadMediaFile(first, "video"));
                    if (video) {
                    ...
                    const image = await uploadWithFeedback(() => uploadImage(first));
                    if (image) {
        ...
        [createAudioFileNode, createImageFileNode, createVideoFileNode, screenToCanvas, size.height, size.width, uploadWithFeedback],
    );
    ...
    const reloadServerCopy = useCallback(async () => {
        await reloadServerProject(projectId);
        if (useCanvasStore.getState().saveStatusByProject[projectId]?.state === "saved") {
            setServerReloadRevision((value) => value + 1);
        }
    }, [projectId, reloadServerProject]);

    const saveLocalCopy = useCallback(() => {
        const nextId = saveProjectAsNew(projectId);
        if (nextId) navigate(`/canvas/${nextId}`);
    }, [navigate, projectId, saveProjectAsNew]);
        ...
        async (payload: InsertAssetPayload) => {
            try {
                if (payload.kind === "text") {
                    ...
                    const content = payload.storageKey ? await resolveMediaUrl(payload.storageKey, payload.url) : payload.url;
                    const spec = NODE_DEFAULT_SIZE[CanvasNodeType.Audio];
                    const center = screenToCanvas((containerRef.current?.getBoundingClientRect().left || 0) + size.width / 2, (containerRef.current?.getBoundingClientRect().top || 0) + size.height / 2);
                    const id = `audio-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
                    setNodes((prev) => [...prev, { id, type: CanvasNodeType.Audio, title: payload.title, position: { x: center.x - spec.width / 2, y: center.y - spec.height / 2 }, width: spec.width, height: spec.height, metadata: { content, storageKey: payload.storageKey, status: NODE_STATUS_SUCCESS, mimeType: payload.mimeType, bytes: payload.bytes, durationMs: payload.durationMs } }]);
                    setSelectedNodeIds(new Set([id]));
                    ...
                    setNodes((prev) => [
                        ...prev,
                        {
                            ...
                            width: nextSize.width,
                            height: nextSize.height,
                            metadata: { content, storageKey: payload.storageKey, status: NODE_STATUS_SUCCESS, naturalWidth: payload.width, naturalHeight: payload.height },
                        },
                    ]);
                    setSelectedNodeIds(new Set([id]));
                } else {
                ...
                const reason = error instanceof Error ? error.message : t("canvas.projectPage.assetInsertFailedFallback");
                message.error(t("canvas.projectPage.assetInsertFailed", { reason }));
            }
        },
    ...
    return (
        <main className="canvas-project-main flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            ...
            <section className="canvas-project-stage relative min-w-0 flex-1 overflow-hidden">
                <CanvasTopBar
                    ...
                    onSaveNow={() => void flushProject(projectId)}
                    onRetrySave={() => void retryProject(projectId)}
                    onReloadServer={() => void reloadServerCopy()}
                    onSaveCopy={saveLocalCopy}
                    onHome={() => host.navigate(host.routeMode === "admin" ? "/admin/dashboard" : "/dashboard")}
                ...
                <CanvasNodeHoverToolbar
                    ...
                    onFocusedEdit={(node) => navigate(`/editor/${encodeURIComponent(projectId)}/${encodeURIComponent(node.id)}`)}
